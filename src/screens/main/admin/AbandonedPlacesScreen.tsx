...
import {usePlacesHooks} from '../../../controllers/abandonedPlacesController'
import {calculateDistance, verifyPermissions} from '../../../../utils'
import {Header, Loading} from '../../../components'
import {color} from '../../../constants'

interface AbandonedPlacesScreenProps {
  navigation: {
    openDrawer: Function
    navigate: Function
  }
...
const AbandonedPlacesScreen: React.FC<AbandonedPlacesScreenProps> = ({
  navigation,
  ...props
}) => {
  const [abandonedPlaces, setAbandonedPlaces] = useState<AbandondedPlacesList>(
    []
  )

  const {loading, places} = usePlacesHooks()

  const [isFetching, setIsFetching] = useState(false)
  const [userLocation, setUserLocation] = useState<PickedLocationProps>()
  ...
  return (
    <View style={styles.container}>
      <Header
        ...
        headerRight={
          <TouchableOpacity
            onPress={() => navigation?.navigate('AbandonedPlacesEditPage')}
          >
            <MaterialCommunityIcons
      ...
      <View style={styles.body}>
        <FlatList
          ...
          renderItem={({item}) => (
            <TouchableOpacity
              onPress={() => {
                navigation.navigate('AbandonedPlaceDetails', {
                  item,
                })
              }}
            >
              ...
              >
                ...
                >
                  <Text
                    style={{fontSize: 24, fontFamily: 'open-sans-semi-bold'}}
                  >
                    {item.name}
                  </Text>
                  <Text
                    numberOfLines={2}
                    style={{fontSize: 18, fontFamily: 'open-sans-regular'}}
                  >
                    {item.description}
                  </Text>
                </View>
