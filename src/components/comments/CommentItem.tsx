import firebase from 'firebase'
import {Feather, Ionicons, MaterialCommunityIcons} from '@expo/vector-icons'
import React, {useState, useEffect} from 'react'
import {
  StyleSheet,
  View,
  Text,
  TouchableOpacity,
  TextInput,
  Image,
} from 'react-native'
import {color} from '../../constants'
import {ButtonWithIcon} from '../ButtonWithIcon'

import moment from 'moment'
import {AbandonedPlaceModel, CommentModel, Rating, UserModel} from '../../redux'
import {
  updateComment,
  updateCommentOfComment,
} from '../../controllers/commentController'
import {useNavigation} from '../../../utils'

interface CommentItemProps {
  isReply?: boolean
  isThreadPage?: boolean
  onShowPopUp: (comment: CommentModel) => void
  mainComment: CommentModel
  replyComment?: CommentModel
  abandonedPlaceID?: string
  user: UserModel
  navigation?: {
    popToTop: Function
    setOptions: Function
    goBack: Function
    navigate: Function
  }
}

const CommentItem: React.FC<CommentItemProps> = ({
  isReply = false,
  isThreadPage = false,
  onShowPopUp,
  mainComment,
  replyComment,
  abandonedPlaceID,
  user,
}) => {
  const {navigate} = useNavigation()

  const isMainOrReplyComment = isReply ? replyComment : mainComment

  const imageSize = isReply ? 30 : 60
  const {likes, dislikes} = isMainOrReplyComment?.rating

  console.log('is this reply?', isReply, mainComment.id)

  const isLiked = likes.some((userId) => userId === user.id)
  const isDisliked = dislikes.some((userId) => userId === user.id)

  const [repliesCount, setRepliesCount] = useState<number>(0)

  useEffect(() => {
    const commentsListener = firebase
      .firestore()
      .collection('abandonedPlaces')
      .doc(abandonedPlaceID)
      .collection('comments')
      .doc(mainComment.id)
      .collection('replyComments')
      .onSnapshot((querySnapshot) => {
        const comments = querySnapshot.docs.map((doc) => {
          return {
            ...doc.data(),
          }
        })

        setRepliesCount(comments.length)
      })

    return () => commentsListener()
  }, [])

  const onLikeComment = async () => {
    if (isReply) {
      replyComment?.rating?.likes.push(user.id)
      const newArr = replyComment?.rating?.dislikes.filter(
        (userId) => userId !== user.id
      )

      replyComment.rating.dislikes = newArr

      await updateCommentOfComment(abandonedPlaceID, mainComment, replyComment)
    } else {
      mainComment.rating.likes.push(user.id)
      const newArr = mainComment.rating.dislikes.filter(
        (userId) => userId !== user.id
      )
      mainComment.rating.dislikes = newArr
      await updateComment(abandonedPlaceID, mainComment)
    }
  }

  const onDislikeComment = async () => {
    if (isReply) {
      replyComment?.rating?.dislikes.push(user.id)
      const newArr = replyComment.rating.likes.filter(
        (userId) => userId !== user.id
      )
      replyComment.rating.likes = newArr
      await updateCommentOfComment(abandonedPlaceID, mainComment, replyComment)
    } else {
      mainComment.rating.dislikes.push(user.id)
      const newArr = mainComment.rating.likes.filter(
        (userId) => userId !== user.id
      )
      mainComment.rating.likes = newArr
      await updateComment(abandonedPlaceID, mainComment)
    }
  }

  return (
    <View
      style={{
        flexDirection: 'row',
        paddingHorizontal: 10,
        paddingTop: 10,
      }}
    >
      <View style={{paddingRight: 10}}>
        <Image
          resizeMode={'cover'}
          style={{height: imageSize, width: imageSize, borderRadius: 100}}
          source={{
            uri: 'data:image/jpeg;base64,',
          }}
        />
      </View>
      <View
        style={{
          justifyContent: 'space-between',
          height: 100,
          marginBottom: 10,
        }}
      >
        <View
          style={{
            flexDirection: 'row',
          }}
        >
          <View>
            <Text>{isMainOrReplyComment?.username},</Text>
          </View>
          <View style={{paddingLeft: 4}}>
            <Text>
              {moment(isMainOrReplyComment?.createdAt?.toDate()).fromNow()}
            </Text>
          </View>
        </View>
        <View style={{flexDirection: 'row'}}>
          <View>
            <Text>{isMainOrReplyComment?.commentText}</Text>
          </View>
        </View>
        <View
          style={{
            flexDirection: 'row',
            justifyContent: 'space-between',
            width: 250,
          }}
        >
          <View>
            <TouchableOpacity
              onPress={() =>
                navigate('HomeAbandonedPlaceThreadPage', {
                  abandonedPlaceID,
                  mainComment,
                })
              }
            >
              <Text>Reply</Text>
            </TouchableOpacity>
          </View>
          <View>
            <ButtonWithIcon
              icon={
                <MaterialCommunityIcons
                  name={'arrow-up-thick'}
                  size={20}
                  color={
                    isLiked ? color.greyedOutColor : color.secondaryColor
                    // likeIsToggled ? color.greyedOutColor : color.secondaryColor
                  }
                />
              }
              disabled={isLiked}
              iconText={isMainOrReplyComment?.rating?.likes.length}
              onTap={onLikeComment}
              otherStyle={{
                height: 20,
              }}
            />
          </View>
          <View>
            <ButtonWithIcon
              icon={
                <MaterialCommunityIcons
                  name={'arrow-down-thick'}
                  size={20}
                  color={
                    isDisliked ? color.greyedOutColor : color.secondaryColor
                  }
                />
              }
              disabled={isDisliked}
              iconText={isMainOrReplyComment?.rating?.dislikes.length}
              onTap={onDislikeComment}
              otherStyle={{
                height: 20,
              }}
            />
          </View>
          <View>
            <ButtonWithIcon
              icon={
                <Feather
                  name={'more-horizontal'}
                  size={20}
                  color={color.secondaryColor}
                />
              }
              onTap={() => onShowPopUp(isMainOrReplyComment)}
              otherStyle={{
                height: 20,
              }}
            />
          </View>
        </View>
        {repliesCount !== 0 && !isReply && (
          <View
            style={{
              flexDirection: 'row',
            }}
          >
            <View>
              <ButtonWithIcon
                icon={
                  <Ionicons
                    name={'caret-down-sharp'}
                    size={16}
                    color={color.secondaryColor}
                  />
                }
                onTap={() =>
                  navigate('HomeAbandonedPlaceThreadPage', {
                    abandonedPlaceID,
                    mainComment,
                  })
                }
                otherStyle={{
                  height: 20,
                }}
              />
            </View>
            <View style={{paddingLeft: 6}}>
              <Text>
                View {repliesCount} {repliesCount > 1 ? 'replies' : 'reply'}
              </Text>
            </View>
          </View>
        )}
      </View>
    </View>
  )
}

const styles = StyleSheet.create({
  container: {flex: 1, backgroundColor: 'green'},
  navigation: {flex: 2, backgroundColor: 'red'},
  body: {
    flex: 10,
    justifyContent: 'center',
    alignItems: 'center',
    backgroundColor: 'yellow',
  },
  footer: {flex: 1, backgroundColor: 'cyan'},
})

export {CommentItem}
